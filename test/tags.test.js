...
describe('Tag Router Tests', () => {
  ...
  const req = (method, endpoint) => {
    ...
    return chai.request(app)[method]('/api/tags' + endpoint).set('Authorization', `Bearer ${token}`);
  };
  ...
  describe('GET /api/tags', function() {
    it('should respond with all tags', () => {
      ...
      return (
        Promise.all([Tag.find({ userId: user.id }), req('get', '/')])
    ...
    it('should sort tags by name', () => {
      return req('get', '/').then(dbRes => {
        expect(dbRes.body.map(item => item.name)).to.eql(dbRes.body.map(item => item.name).sort());
      });
    });
  });
